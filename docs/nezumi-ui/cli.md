...
| `--defaults`            | `-d`  | Use defaults (`--template=next --preset=new-york`)       | `false` |
...
| `style`              | `string`  | Visual style (e.g. `new-york`)                                                         |
...
| `aliases.utils`      | `string`  | Utils import alias (e.g. `@nezumi/ui/lib/utils`)                                                    |
...
1. **Named:** `--preset new-york` or `--preset new-york`
2. **Code:** `--preset a2r6bw` (version-prefixed base62 string, e.g. `a2r6bw` or `b0`)
3. **URL:** `--preset "https://ui.shadcn.com/init?base=radix&style=new-york&..."`
