...
- **`style`** → component visual treatment (e.g. `new-york`).
...
6. **Fix imports in third-party components** — After adding components from community registries (e.g. `@bundui`, `@magicui`), check the added non-UI files for hardcoded import paths like `@/components/ui/...`. These won't match the project's actual aliases. Use `npx shadcn@latest info` to get the correct `ui` alias and rewrite public imports to leaf paths such as `@packages/ui/components/button`. The CLI rewrites imports for its own UI files, but third-party registry components may use default paths that don't match the project.
...
npx shadcn@latest init --name my-app --preset new-york
npx shadcn@latest init --name my-app --preset a2r6bw --template vite

# Create a monorepo project.
npx shadcn@latest init --name my-app --preset new-york --monorepo
npx shadcn@latest init --name my-app --preset new-york --template next --monorepo

# Initialize existing project.
npx shadcn@latest init --preset new-york
npx shadcn@latest init --defaults  # shortcut: --template=next --preset=new-york (base style implied)
...
**Named presets:** `new-york`, `new-york`, `new-york`, `new-york`, `new-york`
