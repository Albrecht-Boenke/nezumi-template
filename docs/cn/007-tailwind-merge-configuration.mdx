---
title: tailwind-merge — Configuration
description: Installation, custom Tailwind config, extendTailwindMerge, plugins; upstream doc mirror.
---
...
If you're using Tailwind CSS without any extra config, you can use [`twMerge`](./009-tailwind-merge-api-reference.mdx#twmerge) right away. You can safely stop reading the documentation here.

## Usage with custom Tailwind config {#usage-with-custom-tailwind-config}

If you're using a custom Tailwind config, you may need to configure tailwind-merge as well to merge classes properly.

The default [`twMerge`](./009-tailwind-merge-api-reference.mdx#twmerge) function is configured in a way that you can still use it if all the following points apply to your Tailwind config:
...
Because the function is under the `fill` key, it will only get called for classes which start with `fill-`. Also, the function only gets passed the part of the class name which comes after `fill-`, this way you can use the same function in multiple class groups. tailwind-merge exports its own [validators](./009-tailwind-merge-api-reference.mdx#validators), so you don't need to recreate them.
...
If you only need to slightly modify the default tailwind-merge config, [`extendTailwindMerge`](./009-tailwind-merge-api-reference.mdx#extendtailwindmerge) is the easiest way to extend the config. You provide it a `configExtension` object which gets [merged](./009-tailwind-merge-api-reference.mdx#mergeconfigs) with the default config. Therefore, all keys here are optional.
...
If you need to modify the tailwind-merge config and need more control than [`extendTailwindMerge`](./009-tailwind-merge-api-reference.mdx#extendtailwindmerge) gives you or don't want to use the default config (and tree-shake it out of your bundle), you can use [`createTailwindMerge`](./009-tailwind-merge-api-reference.mdx#createtailwindmerge).
...
You can use both [`extendTailwindMerge`](./009-tailwind-merge-api-reference.mdx#extendtailwindmerge) and [`createTailwindMerge`](./009-tailwind-merge-api-reference.mdx#createtailwindmerge) with third-party plugins. Just add them as arguments after your config.
...
Next: [Recipes](./008-tailwind-merge-recipes.mdx)

Previous: [Limitations](./006-tailwind-merge-limitations.mdx)
