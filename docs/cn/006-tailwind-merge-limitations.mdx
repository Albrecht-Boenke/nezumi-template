---
title: tailwind-merge — Limitations
description: Edge cases, ambiguous classes, arbitrary properties; upstream doc mirror.
---
...
## You need to use labels in ambiguous arbitrary value classes {#you-need-to-use-labels-in-ambiguous-arbitrary-value-classes}
...
If you have custom classes created with `@apply` or custom CSS that conflict with Tailwind classes, see the [recipes documentation](./008-tailwind-merge-recipes.mdx#extracting-classes-with-tailwinds-apply) for alternatives.
...
Next: [Configuration](./007-tailwind-merge-configuration.mdx)

Previous: [Features](./005-tailwind-merge-features.mdx)
