---
title: tailwind-merge — Recipes
description: Common patterns, custom scales, @apply; upstream doc mirror.
---
...
We'll be able to do this by creating a custom `twMerge` function with [`extendTailwindMerge`](./009-tailwind-merge-api-reference.mdx#extendtailwindmerge).
...
Then we check whether our particular theme scale is included in tailwind-merge's theme config object [here](./007-tailwind-merge-configuration.mdx#theme). Because tailwind-merge supports Tailwind's `shadow` theme scale, we can add it to the tailwind-merge config like this:
...
## Extracting classes with Tailwind's [`@apply`](https://tailwindcss.com/docs/reusing-styles#extracting-classes-with-apply) {#extracting-classes-with-tailwinds-apply}
...
Next: [API reference](./009-tailwind-merge-api-reference.mdx)

Previous: [Configuration](./007-tailwind-merge-configuration.mdx)
