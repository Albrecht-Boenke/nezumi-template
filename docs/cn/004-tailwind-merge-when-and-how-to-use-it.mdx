...
description: Trade-offs, usage patterns, alternatives; upstream doc mirror.
---
...
If you want to merge classes that are all defined within a component, prefer using the [`twJoin`](./009-tailwind-merge-api-reference.mdx#twjoin) function over [`twMerge`](./009-tailwind-merge-api-reference.mdx#twmerge). As the name suggests, `twJoin` only joins the class strings together and doesn't deal with conflicting classes.
...
You don't need to worry about potentially expensive re-renders here because tailwind-merge [caches results](./005-tailwind-merge-features.mdx#results-are-cached) so that a re-render with the same props and state becomes computationally lightweight as far as the call to `twMerge` goes.

If you use a custom Tailwind CSS config, don't forget to [configure tailwind-merge](./007-tailwind-merge-configuration.mdx#usage-with-custom-tailwind-config) as well.
...
Next: [Features](./005-tailwind-merge-features.mdx)

Previous: [What is it for](./003-tailwind-merge-what-is-it-for.mdx)
